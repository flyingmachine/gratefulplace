...
(deftest path-test
  (testing "Uses url key to generate url"
    (is (= (path {:username "chestnut"} :username "users") "/users/chestnut")))

  (testing "Uses record itself when url key doesn't apply"
    (is (= (path "chestnut" :username "users") "/users/chestnut")))

  (testing "Appends suffixes"
    (is (= (path "chestnut" :username "users" "posts") "/users/chestnut/posts"))
    (is (= (path "chestnut" :username "users" "posts" "all") "/users/chestnut/posts/all"))))

;; TODO could make this less complex
(deftest set-path-test
  (testing "Sets the :href attribute of a node"
    (is (sets-attr? ((set-path "chester" user-path) node) :href "/users/chester"))))

(deftest md-content-test
  (testing "Sets node content to markdownified string"
    (is (sets-content? ((md-content "# h1 test") node) "<div><h1> h1 test</h1></div>"))))
